...
void Bombs::set_position(float x, float y, float z, float rotation_x, float rotation_y, float rotation_z) {
	...
	// this->rotation = glm::vec3(0, rotation_y, 0);
}
...
void Bombs::move() {
	if(this->position.y < -2.0) {
		...
		// this->position.x -= (this->speed * (sin(this->rotation.y * M_PI / 180.0)));
		// this->position.z += (this->speed * (cos(this->rotation.y * M_PI / 180.0)));
	}
}
